...
;     December, 2022    Viviano: added bbl
;     March, 2023       Viviano: added default bands, spectra names
;
; :DISCLAIMER:
...
FUNCTION CSI_READ_ENVI_HEADER, filename
...
;-
         ...
         CASE STRLOWCASE( name ) OF
            ...
            "default bands": IF multilineComplete THEN header.default_bands = STRSPLIT( multilineValue, ",", /EXTRACT )
            ...
            "spectra names": IF multilineComplete THEN header.spectra_names = PTR_NEW( STRSPLIT( multilineValue, ",", /EXTRACT ) )
            "wavelength": IF multilineComplete THEN header.wavelength = PTR_NEW( DOUBLE( STRSPLIT( multilineValue, ",", /EXTRACT ) ) )
            "bbl": IF multilineComplete THEN header.bbl = PTR_NEW( LONG( STRSPLIT( multilineValue, ",", /EXTRACT ) ) )
            ELSE:
         ENDCASE
